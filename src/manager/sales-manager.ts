...
/**
 * Recovers the fromStage/toStage a `create_controlled_experiment`
 * escalation was raised for, out of its own `action` text — the exact
 * format src/manager/intervention-generator.ts generates it in ("...at
 * the X → Y step."). Safe to parse: that string is built by this repo's
 * own code from real stage names, never by a model, so it's a fixed
 * shape, not free text. Returns null if the format ever changes without
 * this parser being updated too.
 */
function parseExperimentStages(action: string): { fromStage: string; toStage: string } | null {
  const match = action.match(/at the (.+?) → (.+?) step\.?\s*$/);
  return match ? { fromStage: match[1], toStage: match[2] } : null;
}
...
export class SalesManager {
  ...
  private async executeApprovedEscalations(now: Date): Promise<void> {
    ...
    for (const escalation of await this.deps.escalations.listApproved()) {
      const executor = this.executorFor(escalation);
      if (!executor) {
      ...
      try {
        ...
        await this.deps.escalations.markExecuted(escalation.id, note, now);
      } catch (err) {
  ...
  /**
   * One executor per capability an escalation can be approved for — `null`
   * for anything unrecognized, so executeApprovedEscalations fails loud
   * rather than silently doing nothing. Each returns the note recorded
   * against the now-executed escalation.
   */
  private executorFor(escalation: ManagerEscalation): (() => Promise<string>) | null {
    if (escalation.capability === "discover_prospects") {
      return async () => {
        ...
        return `Ran discovery at ${execution.maxResults} results per area across ${execution.runs.length} area(s); ${found} candidate(s) found.`;
      };
    }

    if (escalation.capability === "create_controlled_experiment") {
      // fromStage/toStage aren't columns on the escalation — they're
      // parsed back out of `action`'s own fixed wording (generated by
      // src/manager/intervention-generator.ts, never by a model), the
      // same string Gavin already read and approved.
      const stages = parseExperimentStages(escalation.action);
      if (!stages) return null;
      return async () => {
        const execution = await this.executeExperiment(
          {
            action: escalation.action,
            capability: escalation.capability,
            proposedChangePercent: escalation.proposedChangePercent ?? undefined,
            expectedImpact: escalation.expectedImpact ?? 0,
            confidence: 0,
            risk: escalation.risk ?? 0,
          },
        ...
        return execution
          ? `Created experiment "${execution.name}" (${execution.experimentId}).`
          : `An experiment for the ${stages.fromStage} → ${stages.toStage} step was already running — nothing new to create.`;
      };
    }
